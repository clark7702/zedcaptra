...
function IMFCode({ user }: { readonly user: User }) {
  ...
  const onSubmit = (data) => {
    ...
    if (data.imf.toLowerCase() === user?.imfCode.toLowerCase()) {
      ...
      setTimeout(() => {
        setAlertMessage("IMF code has been validated successfully");
      ...
      setAlertMessage("Invalid IMF code");
      ...
      <Card variant="outlined" className="mx-4 md:mx-6">
        <div>
          <div className="bg-primary text-white flex justify-between items-stretch px-7 py-4 md:px-12 font-semibold">
            <h1>International Monetary Fund (IMF) Code</h1>
          ...
          >
            <div className="flex flex-col space-y-8">
              <h1>
                ...
                <span className="text-green-600">{transfer?.country}</span>, You
                will need to provide an IMF code to complete this transfer.
              </h1>

              <h1>
                Please enter the IMF code to complete your transfer. Contact
                customer support 24/7 via{" "}
